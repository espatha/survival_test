				...
				for j in world_gen.chunks:
					if i[0] == j.X and i[1] == j.Y:
						...
						for x in range(16):
							for y in range(16):
								if j["Objects"][y][x][0]:
									var new_obj = name_to_obj[j["Objects"][y][x][0]].instantiate()
...
func _on_nav_update_timeout():
	...
	'''var na = nav_mesh_pos(global_position.x - 0.5, global_position.y - 0.5)
	print(nav_mesh[na[1]][na[0]])'''
